...
# class Detection:
#     def __init__(self, coords, category, conf, metadata, imx500: IMX500, picam2: Picamera2):
...
#         self.max_detections = 10 # Maximum number of detections
...
#         # Load the model to the IMX500
#         self.imx500_active_model = IMX500(model_path)

#         # Network intrinsics - e.g. labels, input size, postprocessing type, task, etc.
#         self.intrinsics = self.imx500_active_model.network_intrinsics
...
#         if not self.intrinsics.labels and labels_path:
...
#         self.picam2 = Picamera2(self.imx500_active_model.camera_num) # Create Picamera2 instance with the IMX500 camera number

#         # Configure Picamera2
#         self.main = {'format': 'RGB888'} # format main stream as 24-bit color
#         config = self.picam2.create_preview_configuration(self.main, controls={"FrameRate": self.intrinsics.inference_rate}, buffer_count=12)

#         self.imx500_active_model.show_network_fw_progress_bar()

#         self.picam2.start(config, show_preview=True)
#         if self.intrinsics.preserve_aspect_ratio:
#             self.imx500_active_model.set_auto_aspect_ratio()
...
#     #     # Check for outpout, if none return none
...
#     #     if metadata:
#     #         async_result = self.pool.apply_async(self.__parse_detections, (metadata,))
...
#         a = 1 + 1
#         # camera_controller.capture_and_queue()
...
class RPICameraController:
    def __init__(self, model_path, labels_path=None, iou=0.65, threshold=0.55, max_detections=10):
        ...
        self.iou = iou  # Intersection over Union
        self.threshold = threshold  # Confidence threshold
        self.max_detections = max_detections  # Maximum number of detections
    ...
    def start_camera(self):
        ...
        if self.imx500_active_model is None:
